...
namespace BehaviorTree.Editor
{
    ...
    public class GraphEditorWindow : EditorWindow, IHasCustomMenu
    {
        ...
        protected virtual string InfoText => "Welcome to BehaviorTree, an ultra-simple behavior tree !";
        ...
        [MenuItem("Tools/BehaviorTree/BehaviorTreeEditor")]
        private static void ShowEditorWindow()
        {
            string path = EditorUtility.SaveFilePanel("Select ScriptableObject save path", Application.dataPath, "BehaviorTreeAsset", "asset");
        ...
        protected virtual BehaviorTreeView StructGraphView(IBehaviorTree bt)
        {
            ...
            infoView.styleSheets.Add(Resources.Load<StyleSheet>("Data/Info"));
        ...
        protected IBehaviorTree LoadDataFromFile(string path)
        {
            try
            {
                return AssetDatabase.LoadAssetAtPath<BehaviorTreeAsset>($"Assets/{path}");

            }
            catch
            {
                ShowNotification(new GUIContent($"Invalid Path:Assets/{path}, please pick ScriptableObject inherited from BehaviorTreeAsset"));
                return null;
            }
        }
    }
