...
namespace BehaviorTree.Editor
{
    ...
    {
        [MenuItem("Tools/BehaviorTree/BehaviorTree Service")]
        private static void ShowEditorWindow()
        {
            GetWindow<ServiceEditorWindow>("BehaviorTree Service");
        }
        ...
        private void OnEnable()
        {
            ...
            searchCache.allBehaviorTreeAssetCache = BehaviorTreeSearchUtility.GetAllBehaviorTreeAssets();
        ...
        private void DrawSerializationService()
        {
            ...
            {
                if (EditorUtility.DisplayDialog("User Tips", "Deserialize all files will change behaviorTreeAsset on the left, make sure to have a backup", "Still Deserialize", "Cancel"))
                {
                    foreach (var pair in serviceData.serializationCollection.serializationPairs)
                    {
                        ...
                        {
                            pair.behaviorTreeAsset.Deserialize(pair.serializedData.text);
        ...
        private void DrawSearchingService()
        {
            ...
            if (searchType != searchCache.searchType || newName != searchCache.searchName)
            {
                ...
                else
                {
                    ...
                    searchCache.searchResult = BehaviorTreeSearchUtility.SearchBehaviorTreeAsset(searchType, serviceData, searchCache.allBehaviorTreeAssetCache);
        ...
        private void ExportAll()
        {
            ...
            for (int i = 0; i < serviceData.serializationCollection.serializationPairs.Count; i++)
            {
                ...
                var serializedData = SerializeUtility.SerializeTree(pair.behaviorTreeAsset, true, true);
