...
namespace BehaviorTree.Editor
{
    ...
    {
        public static IEnumerable<IGrouping<string, Type>> GroupsByBtGroup(this IEnumerable<Type> types)
        {
            return types.GroupBy(t =>
            {
                var array = t.GetCustomAttributes(typeof(BtGroupAttribute), false) as BtGroupAttribute[];
        ...
        {
            return groups.SelectMany(x => x).Where(x => x.IsSubclassOf(Father)).GroupsByBtGroup();
        }
        ...
        public static void AddEntry(this List<SearchTreeEntry> entries, Type _type, int _level, Texture icon)
        {
            ...
            var array = _type.GetCustomAttributes(typeof(BtLabelAttribute), false) as BtLabelAttribute[];
